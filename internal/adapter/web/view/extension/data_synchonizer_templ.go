...
func DataSynchronizer(stripePublishableKey string, projectId, extensionId string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if templ_7745c5c3_Err != nil {
			return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/adapter/web/view/extension/data_synchonizer.templ`, Line: 11, Col: 130}
		}
		...
		if templ_7745c5c3_Err != nil {
			return templ.Error{Err: templ_7745c5c3_Err, FileName: `internal/adapter/web/view/extension/data_synchonizer.templ`, Line: 21, Col: 58}
		}
		_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString(templ.EscapeString(templ_7745c5c3_Var3))
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		_, templ_7745c5c3_Err = templ_7745c5c3_Buffer.WriteString("\" hx-target=\"#dashboard-content\"></button></div><div class=\"vr opacity-20 my-1\"></div><h1 class=\"h4 ls-tight\">Data Synchronizer</h1></div></div><div class=\"col\"><div class=\"hstack gap-2 justify-content-end\"><button type=\"button\" @click=\"createSubscription\" class=\"btn btn-sm btn-neutral d-none d-sm-inline-flex\"><span class=\"pe-2\"><i class=\"bi bi-plus-circle\"></i></span><span>Subscription</span></button></div></div></div></header></form></main><script>\r\n\t\tfunction subscriptionForm(stripePublishableKey,projectId,extensionId) {\r\n\t\t\treturn {\r\n\r\n\t\t\t\tcreateSubscription() {\r\n\t\t\t\t\tconst stripe = Stripe(stripePublishableKey);\r\n\r\n\t\t\t\t\tconst value = {\r\n\t\t\t\t\t\t\r\n\t\t\t\t\t\tprojectId: projectId, // Add additional data if needed\r\n\t\t\t\t\t\textensionId: extensionId // Add additional data if needed\r\n\t\t\t\t\t};\r\n\r\n\t\t\t\t\tfetch(\"/payment\", {\r\n\t\t\t\t\t\tmethod: 'POST',\r\n\t\t\t\t\t\theaders: {\r\n\t\t\t\t\t\t\t'Content-Type': 'application/json'\r\n\t\t\t\t\t\t},\r\n\t\t\t\t\t\tbody: JSON.stringify(value)\r\n\t\t\t\t\t})\r\n\t\t\t\t\t.then(async response => {\r\n\t\t\t\t\t\tconst res = await response.json();\r\n\t\t\t\t\t\tconst id = res.id;\r\n\t\t\t\t\t\tstripe.redirectToCheckout({ sessionId: id });\r\n\t\t\t\t\t})\r\n\t\t\t\t\t.catch(error => {\r\n\t\t\t\t\t\tconsole.error('Error creating subscription:', error);\r\n\t\t\t\t\t});\r\n\t\t\t\t}\r\n\t\t\t};\r\n\t\t}\r\n\t</script></div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		return templ_7745c5c3_Err
