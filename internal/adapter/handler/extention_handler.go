...
import (
	...
	ru "github.com/stelgkio/otoo/internal/adapter/web/view/extension/redcourier"
	...
	rcu "github.com/stelgkio/otoo/internal/adapter/web/view/project/settings/redcourier"
...
func (dh *DashboardHandler) StripeSuccesRedirect(ctx echo.Context) error {
	...
	if extension.Code == "redcourier" {
		dh.extensionSvc.CreateProjectExtension(ctx, projectID, extension, 30, "")
		return util.Render(ctx, et.ExtentionRedCourierSubscriptionSuccessTemplate(user, project.Name, projectID, extensionID))
	}
...
// Courier4u get extention courier page
func (dh *DashboardHandler) RedCourier(ctx echo.Context) error {
	projectID := ctx.Param("projectId")
	extension, err := dh.extensionSvc.GetExtensionByCode(ctx, "redcourier")
	if err != nil {
		return err
	}
	//TODO:  by projectID and extensionID find AcsCourierExtension
	acs, err := dh.extensionSvc.GetRedCourierProjectExtensionByID(ctx, extension.ID.Hex(), projectID)
	if err != nil {
		return err
	}
	if acs == nil {
		acs = new(domain.RedCourierExtension)
	}

	return util.Render(ctx, ru.RedCourier(projectID, extension.ID.Hex(), nil, acs))
}

// Courier4uFormPost post form with acs courier data
func (dh *DashboardHandler) RedCourierFormPost(ctx echo.Context) error {
	projectID := ctx.Param("projectId")
	extension, err := dh.extensionSvc.GetExtensionByCode(ctx, "redcourier")
	if err != nil {
		return err
	}
	req := new(domain.RedCourierExtension)
	if err := ctx.Bind(req); err != nil {
		slog.Error("Create project binding error", "error", err)
		return util.Render(ctx, ru.RedCourier(projectID, extension.ID.Hex(), nil, req))
	}
	validationErrors := req.Validate()
	if len(validationErrors) > 0 {
		return util.Render(ctx, ru.RedCourier(projectID, extension.ID.Hex(), validationErrors, req))
	}
	...
	dh.extensionSvc.CreateRedCourierProjectExtension(ctx, projectID, req)

	return util.Render(ctx, ru.RedCourierSubscriptio(os.Getenv("STRIPE_PUBLICK_KEY"), projectID, extension.ID.Hex()))
}

// Courier4uFormPost post form with acs courier data
func (dh *DashboardHandler) RedCourierettingsFormPost(ctx echo.Context) error {
	projectID := ctx.Param("projectId")
	userID, err := auth.GetUserID(ctx)
	if err != nil {
		return err
	...
	err = dh.extensionSvc.CreateRedCourierProjectExtension(ctx, projectID, req)
	if err != nil {
		return ctx.JSON(http.StatusBadRequest, map[string]string{"error": err.Error()})
	}

	if ctx.Request().Header.Get("HX-Request") == "true" {
		return r.Render(ctx, rcu.SettingsRedCourier(projectID, projectExtensions, user, make(map[string]string), req))
	}
	return r.Render(ctx, stm.RedCourierTemplate(user, project.Name, projectID, project, projectExtensions, make(map[string]string), req))
}

// Courier4uDeActivate post form with acs courier data
func (dh *DashboardHandler) RedCourierDeActivate(ctx echo.Context) error {
	projectID := ctx.Param("projectId")
	extension, err := dh.extensionSvc.GetExtensionByCode(ctx, "redcourier")
	if err != nil {
		return err
